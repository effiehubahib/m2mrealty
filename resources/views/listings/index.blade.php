                        ...
                        <div class="mt-8 flow-root" >
                            <div class="-mx-4 -my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
                                <div class="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8">
                                    <table class="relative min-w-full divide-y divide-gray-300 dark:divide-white/15" id="developer-table">
                                        <thead>
                                            <tr class="dark:text-white">
                                                <th style="width: 45px;" scope="col" class="py-3.5 pr-3 pl-4 text-left text-sm font-semibold ">Logo</th>
                                                <th scope="col" class="py-3.5 pr-3 pl-4 text-left text-sm font-semibold ">Developer Name</th>                                                
                                                <th scope="col" class="py-3.5 pr-3 pl-4 text-center text-sm font-semibold " >Action</th>
                                            </tr>
                                        </thead>
                                            ...
                                            @foreach ($listings as $ctr=>$developer)
                                                <tr>
                                                    ...
                                                    <td class="text-center">
                                                        ...
                                                        | 
                                                        {{ html()->form('DELETE')->route('developer.destroy',$developer->id)->id('deleteForm')->class('inline space-y-6')->open() }}
                                                            <input type="submit" class="deleteBtn cursor-pointer text-red-600 hover:text-red-900 text-sm" value="Delete" onclick="return confirm('Are you sure you want to delete {{$developer->developername}} ?')"/>
                                                        {{ html()->form()->close() }}
                                                    </td>
                                                </tr>
                                            @endforeach
